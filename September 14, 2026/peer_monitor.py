...
def load_vr_mapping(map_path=None, sheet=VR_PEER_SHEET):
    ...
    # ★ THE VR PEER SET IS FIXED — DO NOT CANONICALISE IT (Kyser 2026-09-09: "the VR peerset is
    # fixed dont change that"). CATEGORY_CANON is deliberately NOT applied here. The Value
    # Research peer sets are a ratified construction — their categories and their borrowed-peer
    # widening are decisions, not vendor drift — so folding a name here would silently redraw
    # peer groups he has already signed off. The fold belongs to the MFI all-peer map alone,
    # which is where the Bal Bhavishya mis-tag actually split Children's Fund 10-vs-3.
...
# ── CATEGORY CANON (Kyser 2026-09-09: "Bal Bhavishya doesnt feature in category - it should be
# in children's category") ──────────────────────────────────────────────────────────────────
# A FUND BRAND leaked into the vendor's Category column. "Bal Bhavishya Yojna" is Aditya Birla's
# children's fund; the vendor used that product name as a CATEGORY and filed three schemes under
# it — Aditya Birla's own (AMFI 146407), Tata Childrens Fund (119312) and UTI Childrens Equity
# Fund (120724) — while the other nine children's funds were correctly tagged "Children's Fund".
# All twelve carry the same `Scheme Nature = Solutions`.
#
# It is not cosmetic. A three-fund category cannot be ranked, so the deck widened it by borrowing
# NINETEEN ELSS Tax Saver funds as peers — so those three children's funds were being quartiled
# against tax-saver funds, and Children's Fund itself showed 10 schemes instead of 13.
#
# Fixed HERE and not in the mapping workbook on purpose: that workbook is re-downloaded from the
# vendor, so an edit there is silently undone on the next refresh. This is the same idiom
# HOUSE_CANON already uses for vendor drift. Keyed on the exact vendor string.
CATEGORY_CANON = {"Bal Bhavishya": "Children's Fund"}


def canon_categories(s):
    """Fold vendor category spellings onto the canonical ones. Applied at EVERY point a Category
    column enters the engine, so a stray tag cannot survive in one map and not in another."""
    return s.replace(CATEGORY_CANON)


def audit_category_names(cmap, log=print):
    """THE GUARD for the Bal Bhavishya class: a category name that is really a FUND BRAND.

    The tell is simple and general. A real category name shows up in the scheme names of MANY
    houses — "Flexi Cap Fund" appears in Aditya Birla's, HDFC's, SBI's. A brand appears in the
    names of exactly ONE, because only that house sells it. "Bal Bhavishya" appeared only inside
    "Aditya Birla Sun Life Bal Bhavishya Yojna", and the vendor had made it a category.

    Returns the list of suspect categories and logs each one. Deliberately a WARNING, not a hard
    failure: a genuinely new single-house category (a first-mover launching a real category) would
    trip it, and that must not stop a nightly refresh -- but nobody can say they were not told.
    """
    suspects = []
    for cat, grp in cmap.groupby("Category"):
        key = re.sub(r"\s+", " ", str(cat)).strip().lower()
        if not key or key == "unknown":
            continue
        # every house whose scheme NAMES contain the category name
        houses = {house_of(nm) for nm in cmap.index
                  if key in re.sub(r"\s+", " ", str(nm)).strip().lower()}
        if len(houses) == 1 and len(grp) <= 8:
            suspects.append({"category": str(cat), "n_schemes": int(len(grp)),
                             "only_house": sorted(houses)[0],
                             "members": [str(x) for x in grp.index]})
    for s in suspects:
        log(f"  [category audit] '{s['category']}' looks like a FUND BRAND, not a category: "
            f"its name appears only in {s['only_house']}'s scheme names, and it holds "
            f"{s['n_schemes']} scheme(s). Members: {'; '.join(s['members'][:6])}")
        log("                   -> if it is a brand, add it to CATEGORY_CANON with the category "
            "it really belongs to.")
    if not suspects:
        log("  [category audit] no category name looks like a fund brand.")
    return suspects
...
def build_category_map(raw_map: pd.DataFrame, code2name: dict | None = None):
    ...
    cm["Category"] = canon_categories(cm["Category"])          # vendor drift -> canonical name
    ...
    # Every refresh re-reads the vendor's mapping, so a brand-as-category can come back at any
    # time. Say so on every build rather than waiting for someone to notice a missing fund.
    audit_category_names(cm[["Category"]])
    assigned = set()
...
# Four investor books (1Y and 2Y added 2026-09-08 on Kyser's ask). A window can never exceed the
# book that measures it, so the 1-year book only ever carries the 1M..1Y windows and the 2-year
# book stops at 2Y -- build_sip already enforces that (`if m > book: continue`) and the deck
# already disables the buttons it makes impossible, so adding a book needs no other change.
SIP_BOOKS = {"sip1y": 12, "sip2y": 24, "sip3y": 36, "sip5y": 60}   # payload key -> book length, months
SIP_BOOK_LABEL = {"sip1y": "1-year SIP book", "sip2y": "2-year SIP book",
                  "sip3y": "3-year SIP book", "sip5y": "5-year SIP book"}
...
def write_dashboard_json(path, res, pct, cmap, amc_all, sleeve, exclusions, peer_map,
    ...
    def _series(row):
        # ★ 6 dp, not 4 (2026-09-02). These rows carry BOTH the four quartile bands and the
        # "% of AMC AUM" weight, and the deck divides one by the other. Rounding the numerator
        # and the denominator independently to 4 dp let the ratio drift by up to one rounding
        # step, 1e-4/weight — which on a small sleeve is large: Sundaram/Arbitrage+ (weight
        # 0.0021) printed a Q1+Q2 of 104.76%, and ABSL/Bottoms Up printed the 100.083% Kyser
        # flagged. 3,660 of 38,858 sleeve cells were above 100%, every one by exactly +-0.0001.
        # The deck now divides by the SUM OF THE BANDS (see sleeveAt in _dashboard_src.html), so
        # the shares themselves are immune; this bounds the drift on the one ratio that still
        # spans both quantities — rated AUM / sleeve weight, the coverage figure — to ~1e-6.
        # A share of a house's AUM needs 6 dp to be worth 1 bp of a 1% sleeve: 4 dp was simply
        # below the resolution the arithmetic downstream needed.
    ...
    if sip is not None:
        ...
        # ★ WHICH BOOKS THIS PAYLOAD ACTUALLY CARRIES. The deck offers one mode button per book and
        # gates them on this list, so a deck built before the 1Y/2Y books existed cannot show a
        # button that would sit on "loading SIP data" for ever. (Kyser 2026-09-08.)
        payload["meta"]["sip_books"] = [k for k in sip if isinstance(sip.get(k), dict) and "sipret" in sip[k]]
        payload["sip"] = sip
